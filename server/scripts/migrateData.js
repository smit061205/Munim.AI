...
async function migrateAllData() {
  try {
    const { Asset, Liability, Transaction, EPF, CreditScore, Investment } =
      await import("../models/Financial.js");
    ...
    // Read JSON files
    const assetsData = await readJsonFile(
      path.join(__dirname, "../data/assets.json")
    );
    const liabilitiesData = await readJsonFile(
      path.join(__dirname, "../data/liabilities.json")
    );
    const transactionsData = await readJsonFile(
      path.join(__dirname, "../data/transactions.json")
    );
    const epfData = await readJsonFile(
      path.join(__dirname, "../data/epf.json")
    );
    const creditScoreData = await readJsonFile(
      path.join(__dirname, "../data/creditScore.json")
    );
    const investmentsData = await readJsonFile(
    ...
    const assetDocs = assetsData.user_assets.map((asset) => ({
      ...
      clerkId: asset.user_id, // Use user_id as clerkId for compatibility
    ...
    const liabilityDocs = liabilitiesData.user_liabilities.map((liability) => ({
      ...
      clerkId: liability.user_id, // Use user_id as clerkId for compatibility
      liabilities: liability.liabilities,
    }));
    await Liability.insertMany(liabilityDocs);

    // Migrate transactions
    const transactionDocs = transactionsData.user_transactions.map(
      (transaction) => ({
        user_id: transaction.user_id,
        clerkId: transaction.user_id, // Use user_id as clerkId for compatibility
    ...
    const epfDocs = epfData.user_epf.map((epf) => ({
      user_id: epf.user_id,
      clerkId: epf.user_id, // Use user_id as clerkId for compatibility
      uan: epf.uan,
    ...
    const creditScoreDocs = creditScoreData.user_credit_scores.map((score) => ({
      ...
      clerkId: score.user_id, // Use user_id as clerkId for compatibility
    ...
    // Migrate investments
    const investmentDocs = investmentsData.user_investments.map(
      (investment) => ({
        user_id: investment.user_id,
        clerkId: investment.user_id, // Use user_id as clerkId for compatibility
