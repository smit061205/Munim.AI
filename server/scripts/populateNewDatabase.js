...
const users = [
  {
    clerkId: "user_32eIuzzFAAlZ1JzWbY3Cp7ZBSzr",
    email: "smit.thakkar@example.com",
    firstName: "Smit",
    lastName: "Thakkar",
    profileImage:
      "https://images.clerk.dev/uploaded/img_2nVOdJQZGHtJzwKjOq7xjz8K9mL.png",
  },
  {
    clerkId: "user_32eJ1pICmH7nNCsIuUKgZrdq7yr",
    email: "priya.sharma@example.com",
    firstName: "Priya",
    lastName: "Sharma",
    profileImage:
      "https://images.clerk.dev/uploaded/img_2nVOdJQZGHtJzwKjOq7xjz8K9mM.png",
  },
  {
    clerkId: "user_32eJ6hMlZ1V6ItaNkdLrl950AFR",
    email: "rahul.gupta@example.com",
    firstName: "Rahul",
    lastName: "Gupta",
    profileImage:
      "https://images.clerk.dev/uploaded/img_2nVOdJQZGHtJzwKjOq7xjz8K9mN.png",
  },
];

// Generate comprehensive transaction data for each user
const generateTransactions = (clerkId, userIndex) => {
  const transactions = [];
  const categories = [
    "Food & Dining",
    "Transportation",
  ...
  for (let month = 0; month < 6; month++) {
    ...
    transactions.push({
      clerkId,
      amount: 85000 + userIndex * 15000, // Different salaries per user
      category: "Salary",
...
const generateInvestments = (clerkId, userIndex) => {
  const investments = [];
...
const generateAssets = (clerkId, userIndex) => {
...
const generateLiabilities = (clerkId, userIndex) => {
...
const populateDatabase = async () => {
  try {
    ...
    console.log(" Generating comprehensive financial data...");

    for (let i = 0; i < users.length; i++) {
      const user = users[i];
      console.log(
        `\n Processing data for ${user.firstName} ${user.lastName}...`
      );

      // Generate transactions (300+ per user)
      const transactions = generateTransactions(user.clerkId, i);
      console.log(`  Generated ${transactions.length} transactions`);

      // Generate investments (15-25 per user)
      const investments = generateInvestments(user.clerkId, i);
      console.log(`  Generated ${investments.length} investments`);

      // Generate assets (7 per user)
      const assets = generateAssets(user.clerkId, i);
      console.log(`  Generated ${assets.length} assets`);

      // Generate liabilities (4 per user)
      const liabilities = generateLiabilities(user.clerkId, i);
      console.log(`  Generated ${liabilities.length} liabilities`);
      ...
      // Create investment documents - single document per user with all investments
      const stocks = [];
      const mutualFunds = [];
      let totalPortfolioValue = 0;

      // Separate stocks and mutual funds from the generated investments
      ...
      // Create single investment document with all stocks and mutual funds
      ...
      const epfDoc = new EPF({
        ...
        member_id: `${user.firstName.toUpperCase()}${Math.floor(
          Math.random() * 10000
        )}`,
        employee_contribution: 12000 + i * 2000,
        employer_contribution: 12000 + i * 2000,
        total_balance: 500000 + i * 100000,
        kyc_status: "verified",
      });
      await epfDoc.save();
      ...
      // Create enhanced budget data
      const budgetCategories = [
        {
          category: "Rent",
          budgetAmount: 25000 + i * 5000,
          spentAmount: 25000 + i * 5000,
        },
        {
          category: "Groceries",
          budgetAmount: 8000,
      ...
      console.log(
        `  Created budget with ${budgetCategories.length} categories (₹${totalBudget} total)`
      );
    }
    ...
    console.log(` Transactions: ~${users.length * 270} (6 months of data)`);
    console.log(` Investments: ~${users.length * 20}`);
    console.log(` Assets: ${users.length * 7}`);
    console.log(` Liabilities: ${users.length * 4}`);
    console.log(` Budget Categories: ${users.length * 8}`);
  } catch (error) {
